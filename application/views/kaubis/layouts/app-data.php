...
<body>
	<div id="app">
		<div class="main-wrapper">
			<!-- Navbar -->
			<?php $this->load->view('kaubis/components/navbar') ?>
			<!-- Sidebar -->
			<?php $this->load->view('kaubis/components/sidebar') ?>

			<!-- Main Content -->
			<?php $this->load->view($content) ?>
			<!-- Footer -->
			<?php $this->load->view('kaubis/components/footer') ?>
		</div>
	</div>
	...
	<script src="<?= base_url('assets/template/stisla') ?>/assets/js/page/modules-datatables.js"></script>
	<script>
		$(function() {
			$(document).on('click', '.ktp', function(e) {
				e.preventDefault()
				$('#exampleModal').modal('show')
				let path = '<?= base_url('assets/images/') ?>';
				$('#image').attr('src', path + $(this).data('ktp'))
			})
			$(document).on('click', '.selfie', function(e) {
				e.preventDefault()
				$('#exampleModal').modal('show')
				let path = '<?= base_url('assets/images/') ?>';
				$('#image').attr('src', path + $(this).data('selfie'))
			})
		})
	</script>
